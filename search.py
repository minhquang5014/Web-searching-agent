...
Backend priority:
    ...
    2. Tavily API     — AI-optimised; returns scraped page content alongside URLs
    3. Brave API      — fallback if BRAVE_SEARCH_API_KEY is set
    4. DuckDuckGo     — last-resort, may be rate-limited

API keys are read from a .env file in the working directory (or from env vars).
...
# ──────────────────────────────────────────────────────────────────────
# Tavily API (fallback #1 — returns scraped content alongside URLs)
# ──────────────────────────────────────────────────────────────────────

def tavily_search(query: str, num_results: int) -> SearchReport:
    """
    Search via Tavily API.  Each call uses 1 free credit (basic depth).
    Tavily returns a `content` snippet per result — already scraped, no
    separate fetch needed — which populates SearchResult.content directly.
    Free tier: 1,000 searches / month.
    """
    start = time.time()
    report = SearchReport(query=query, backend="Tavily",
                          requested=num_results, results=[])
    if not TAVILY_API_KEY:
        report.error = "TAVILY_API_KEY not set."
        return report
    try:
        resp = requests.post(
            ...
            json={
                ...
                "query": query,
                "max_results": min(num_results, 20),   # Tavily caps at 20
        ...
        report.error = f"Tavily error: {e}"
    report.elapsed_s = time.time() - start
    return report


# ──────────────────────────────────────────────────────────────────────
# Brave API (fallback #2)
# ──────────────────────────────────────────────────────────────────────
...
def search(query: str, num_results: int = 10,
           base_url: str = SEARXNG_BASE_URL) -> SearchReport:
    """
    Try SearXNG first; fall back through Tavily → Brave → DDG if it returns
    nothing (rate-limited, CAPTCHA, or unreachable).
    """
    report = searxng_search(query, num_results, base_url)
    if report.results:
        return report
    for fallback in (tavily_search, brave_search, ddg_search):
        fb = fallback(query, num_results)
        if fb.results:
            return fb
    return report  # nothing worked — return the original SearXNG error
